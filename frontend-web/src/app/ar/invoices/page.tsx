...
export default function ArInvoicesPage() {
  ...
  const exportCsv = async () => {
    ...
    const res = await fetch(`${API_BASE}/exports/ar/invoices?${params.toString()}`, { headers: { Authorization: `Bearer ${token||''}`, 'X-Org-Id': org } });
    const blob = await res.blob();
    const url = URL.createObjectURL(blob); const a=document.createElement('a'); a.href=url; a.download='ar_invoices.csv'; a.click(); URL.revokeObjectURL(url);
  };
  const [filters, setFilters] = useState<{status?: string; customer_id?: string; date_from?: string; date_to?: string; q?: string}>({});
  const [error, setError] = useState<string | null>(null);
  const [formErrors, setFormErrors] = useState<Record<string,string[]>>({});
  const [form, setForm] = useState<{customer_id?: number; invoice_no?: string; invoice_date?: string; lines: Line[]}>({ lines: [{},{}] });
  const [editModal, setEditModal] = useState<{id?: number; data?: any} | null>(null);
  const [postModal, setPostModal] = useState<{id?: number, ar_account_id?: number} | null>(null);

  const arCandidates = useMemo(() => accounts.filter(a=>a.type==='asset'), [accounts]);
  ...
  const load = () => {
    ...
    Promise.all([
      api(`/customers`),
      api(`/accounts`),
      api(`/ar/invoices${qs?`?${qs}`:''}`),
    ])
      .then(([cust, acc, inv]) => {
        setCustomers(Array.isArray(cust?.data) ? cust.data : cust);
        setAccounts(Array.isArray(acc) ? acc : acc?.data || []);
        if (Array.isArray(inv?.data)) {
          setList(inv.data);
          setPage(inv.current_page || 1);
          setLastPage(inv.last_page || 1);
          setTotal(inv.total || inv.data.length);
        } else {
          setList(inv);
          setLastPage(1);
          setTotal(inv.length || 0);
        }
      })
  ...
  const create = async (e: React.FormEvent) => {
    ...
    try {
      await api('/ar/invoices', { method: 'POST', body: JSON.stringify(form) });
      setForm({ lines: [{},{}] });
      setFormErrors({});
      load();
  ...
  const post = async () => {
    if (!postModal?.id || !postModal?.ar_account_id) return;
    try {
      await api(`/ar/invoices/${postModal.id}/post`, { method: 'POST', body: JSON.stringify({ ar_account_id: postModal.ar_account_id }) });
  ...
  return (
    <div className="max-w-5xl mx-auto p-4">
      <h1 className="text-2xl font-semibold mb-4">AR Invoices</h1>
      ...
      <div className="grid grid-cols-5 gap-2 mb-2">
        ...
        <select value={filters.customer_id||''} onChange={e=>setFilters({...filters, customer_id:e.target.value||undefined})} className="border rounded px-2 py-1">
      ...
      <form onSubmit={create} className="space-y-2 border rounded p-3 mb-6">
        <div className="grid grid-cols-4 gap-2">
          <select required value={form.customer_id||''} onChange={e=>setForm({...form, customer_id: Number(e.target.value)})} className="border rounded px-2 py-1">
            <option value="">Customer</option>
            {customers.map(c=> <option key={c.id} value={c.id}>{c.name}</option>)}
          </select>
          <div>
            <input placeholder="Invoice No" required value={form.invoice_no||''} onChange={e=>setForm({...form, invoice_no:e.target.value})} className="border rounded px-2 py-1 w-full" />
            {formErrors.invoice_no && <p className="text-xs text-red-600">{formErrors.invoice_no.join(', ')}</p>}
          </div>
          <div>
            <input type="date" required value={form.invoice_date||''} onChange={e=>setForm({...form, invoice_date:e.target.value})} className="border rounded px-2 py-1 w-full" />
            {formErrors.invoice_date && <p className="text-xs text-red-600">{formErrors.invoice_date.join(', ')}</p>}
          </div>
        </div>
        <div>
      ...
      <table className="w-full border text-sm">
        <thead><tr className="bg-gray-50"><th className="p-2 border">No</th><th className="p-2 border">Customer</th><th className="p-2 border">Date</th><th className="p-2 border">Total</th><th className="p-2 border">Status</th><th className="p-2 border">Actions</th></tr></thead>
        <tbody>
          {list.map((r: any) => (
            <tr key={r.id}>
              <td className="p-2 border">{r.invoice_no}</td>
              <td className="p-2 border">{r.customer?.name || ''}</td>
              <td className="p-2 border">{r.invoice_date}</td>
              ...
              <td className="p-2 border">
                {r.status==='draft' && (
                  <>
                    ...
                    <button onClick={async()=>{ if(confirm('Delete invoice?')){ try{ await api(`/ar/invoices/${r.id}`, { method:'DELETE' }); load(); }catch(e:any){ setError(e.message||'Failed'); } } }} className="text-sm text-red-700 underline">Delete</button>
                  </>
                )}
              </td>
            </tr>
      ...
      {editModal && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center">
          <div className="bg-white p-4 rounded shadow w-full max-w-3xl">
            <h2 className="font-semibold mb-3">Edit Invoice</h2>
            {/* Simple edit: fetch not required, we reuse selected data */}
            <EditInvoiceModal data={editModal.data} accounts={accounts} customers={customers} onClose={()=>setEditModal(null)} onSaved={()=>{setEditModal(null); load();}} setError={setError} />
          </div>
        </div>
      )}
      ...
      {postModal && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center">
          <div className="bg-white p-4 rounded shadow w-full max-w-md">
            <h2 className="font-semibold mb-2">Post Invoice</h2>
            <select value={postModal.ar_account_id||''} onChange={e=>setPostModal({...postModal, ar_account_id: Number(e.target.value)})} className="border rounded px-2 py-1 w-full mb-1">
              <option value="">Select AR Account</option>
              {arCandidates.map(a=> <option key={a.id} value={a.id}>{a.code} - {a.name}</option>)}
            </select>
            {!postModal.ar_account_id && <p className="text-xs text-red-600 mb-2">AR account is required</p>}
            <div className="flex gap-2 justify-end">
              <button onClick={()=>setPostModal(null)} className="px-3 py-1">Cancel</button>
              <button onClick={post} disabled={!postModal.ar_account_id} className={`px-3 py-1 rounded ${!postModal.ar_account_id?'opacity-50 border':'bg-teal-600 text-white'}`}>Confirm</button>
            </div>
          </div>
        </div>
      )}
...
function EditInvoiceModal({ data, accounts, customers, onClose, onSaved, setError }: any) {
  ...
  const save = async (e:any)=>{
    e.preventDefault();
    try { await api(`/ar/invoices/${data.id}`, { method:'PUT', body: JSON.stringify(form) }); setErrors({}); onSaved(); } catch(e:any){ setError(e.message||'Failed'); setErrors(e.errors||{}); }
  };
  return (
    <form onSubmit={save} className="space-y-2">
      <div className="grid grid-cols-3 gap-2">
        ...
        <div>
          <input value={form.invoice_no} onChange={e=>setForm({...form, invoice_no:e.target.value})} className="border rounded px-2 py-1 w-full" />
          {errors.invoice_no && <p className="text-xs text-red-600">{errors.invoice_no.join(', ')}</p>}
        </div>
        <div>
          <input type="date" value={form.invoice_date} onChange={e=>setForm({...form, invoice_date:e.target.value})} className="border rounded px-2 py-1 w-full" />
          {errors.invoice_date && <p className="text-xs text-red-600">{errors.invoice_date.join(', ')}</p>}
        </div>
      </div>
      <table className="w-full border text-sm">
        <thead><tr className="bg-gray-50"><th className="p-1 border">Account</th><th className="p-1 border">Description</th><th className="p-1 border">Qty</th><th className="p-1 border">Unit Price</th><th className="p-1 border">Amount</th></tr></thead>
        <tbody>
          {form.lines.map((ln:any,i:number)=> (
            <tr key={i}>
              <td className="border p-1"><select value={ln.account_id||''} onChange={e=>updateLine(i,{account_id:Number(e.target.value)})} className="border rounded px-1 py-1 w-full"><option value="">Revenue</option>{accounts.map((a:any)=>(<option key={a.id} value={a.id}>{a.code} - {a.name}</option>))}</select></td>
              <td className="border p-1"><input value={ln.description||''} onChange={e=>updateLine(i,{description:e.target.value})} className="border rounded px-1 py-1 w-full"/></td>
              <td className="border p-1"><input type="number" step="0.0001" value={ln.qty||''} onChange={e=>updateLine(i,{qty:Number(e.target.value)})} className="border rounded px-1 py-1 w-full"/></td>
              <td className="border p-1"><input type="number" step="0.01" value={ln.unit_price||''} onChange={e=>updateLine(i,{unit_price:Number(e.target.value)})} className="border rounded px-1 py-1 w-full"/></td>
