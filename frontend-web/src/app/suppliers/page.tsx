...
type Supplier = { id: number; code?: string; name: string; email?: string; phone?: string };
...
export default function SuppliersPage() {
  ...
  const exportCsv = async () => {
    ...
    const res = await fetch(`${API_BASE}/exports/suppliers?${params.toString()}`, { headers: { Authorization: `Bearer ${token||''}`, 'X-Org-Id': org } });
          ...
          {list.map(c=> (
            <tr key={c.id}>
              ...
              <td className="p-2 border text-right">
                ...
                <button className="text-sm text-red-700" onClick={async()=>{ if(confirm('Delete supplier?')){ try{ await api(`/suppliers/${c.id}`, { method: 'DELETE' }); load(); }catch(e:any){ setError(e.message||'Failed'); } } }}>Delete</button>
              </td>
            </tr>
          ))}
        </tbody>
      ...
      {editing && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center">
          <div className="bg-white p-4 rounded shadow w-full max-w-lg">
            ...
            <form onSubmit={async (e)=>{ e.preventDefault(); try{ await api(`/suppliers/${editing.id}`, { method:'PUT', body: JSON.stringify(editing) }); setEditing(null); setEditErrors({}); load(); }catch(e:any){ setError(e.message||'Failed'); setEditErrors(e.errors||{}); } }} className="space-y-2">
              <div>
